...
fn hud_context_from_line(line: &str) -> Option<HudContext> {
    ...
    if !lower.contains("context") && !line.contains("上下文") {
        return None;
    }
    let percent = percent_in_line(line)?;
    let remaining = [
        "left",
        "remaining",
        "available",
    ...
    .iter()
    .any(|needle| lower.contains(needle) || line.contains(needle));
    Some(HudContext {
        percent,
        kind: if remaining {
...
fn percent_in_line(line: &str) -> Option<u8> {
    ...
    for (index, &(byte, ch)) in chars.iter().enumerate() {
        ...
        while start_index > 0 && chars[start_index - 1].1.is_ascii_digit() {
            start_index -= 1;
        }
        ...
        let value = line[start..byte].parse::<u8>().ok()?;
        if value <= 100 {
            return Some(value);
        }
    }
