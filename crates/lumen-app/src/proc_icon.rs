//! F7②：侧栏会话图标 = 会话内**前台运行程序的 exe 图标**（LLM CLI 例外，
//! 见下）。
//!
...
//! 行图标」（pwsh/cmd 的图标）。
//!
//! **例外（2026-08-17 修订）**：识别到受支持的 LLM CLI 时改用 Lumen 内置的
//! 品牌图标，见 [`crate::llm_icon`]。理由是这条规则在那些 CLI 上取不到正确
//! 结果——实测 Claude Code / Codex CLI / Gemini CLI 的前台真进程都是
//! `node.exe`（npm shim），Kimi CLI 是 Node 打包的 `kimi.exe`，四个会顶着
//! 同一个 Node 图标；而 Codex 真正的 `codex.exe` 是 `node.exe` 的孙进程，
//! 本模块只查直接子进程够不到它，且它本身也没嵌资源图标。
//! 「图标不可由用户自定义」不变。
//!
//! 分层：
